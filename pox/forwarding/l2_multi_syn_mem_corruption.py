...
class Switch (EventMixin):
  ...
  def _handle_PortStatus (self, event):
    global memory_corrupted
    if(self.dpid == 1 and event.modified and event.ofp.desc.state & of.OFPPS_LINK_DOWN and event.port == 1):
      log.warning("Port down event, triggering synthetic memory corruption")
      memory_corrupted = True
    elif(self.dpid == 1 and event.modified and not event.ofp.desc.state & of.OFPPS_LINK_DOWN and event.port == 1):
      if memory_corrupted:
        log.warning ("Invoking controller crash aaaaaargh")
        os._exit(1)
      memory_corrupted = True
    else:
      log.warning("Ignoring port status")

...
class FalconReplayer(ControlFlow):
    ...
    def __init__(self, simulation_cfg, superlog_path_or_dag, create_event_scheduler=None,
                 print_buffers=True, wait_on_deterministic_values=False, default_dp_permit=True,
                 ...
                 fail_fast=False, check_interval=500,
                 **kwargs):
        '''
            ...
            for pending_message in messages:
                ...
                if fingerprint not in expected_fingerprints:
                    ...
                    event_type = ControlMessageReceive if type(
                        pending_message) == PendingReceive else ControlMessageSend
...
class DataplaneChecker(object):
    ...
    def update_window(self, current_round):
        ...
        for i in xrange(head_idx, tail_idx):
            if (type(self.events[i]) == DataplanePermit or type(self.events[i]) == DataplaneDrop):

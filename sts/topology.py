...
def create_switch(switch_id, num_ports, can_connect_to_endhosts=True):
    ''' create a switch with some ports, each port is (00:00:00:00:switch_id:port_no, 1.1.switch_id.port_no) '''
...
def get_switchs_host_port(switch):
    ''' Return the switch's ofp_phy_port connected to the host (the last port) '''
...
def create_host(ingress_switch_or_switches, mac_or_macs=None, ip_or_ips=None,
    ...
    '''
    Create a Host, wired up to the given ingress switches using given mac/ip
    or 12:34:56:78:switch.dpid:port.port_no/10.switch.dpid.port.port_no.2
    '''
...
class PatchPanel(object):
    ...
    Listens to the SwitchDPPacketOut event on the switches. It can forward packets to next hop
    or deliver to dst_host directly.
    '''
...
class LinkTracker(object):
    '''
    A class to store all types of links and their statuses and manipulate them
    (e.g. link cut down, repair; host migration)
    '''
...
class Topology(object):
    ...
    the network. It defines several functions to build and manipulate topology entities.
    '''
    ...
    def construct_tree(self, num_pods):
        ...
        for i in range(self.total_hosts):
            ...
            if self.use_portland_addressing:
                ...
                (host, host_access_links) = create_host(edge, mac_or_macs=portland_mac,
                                                        ip_format_str=self.ip_format_str,
